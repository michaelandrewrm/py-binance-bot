def get_symbol_info(client, symbol) -> dict[str, float]:
    ...
    tick_size = float(
        [f for f in info["filters"] if f["filterType"] == "PRICE_FILTER"][0]["tickSize"]
    )
    step_size = float(
        [f for f in info["filters"] if f["filterType"] == "LOT_SIZE"][0]["stepSize"]
    )
    min_qty = float(
        [f for f in info["filters"] if f["filterType"] == "LOT_SIZE"][0]["minQty"]
    )
    min_notional = float(
        [f for f in info["filters"] if f["filterType"] == "NOTIONAL"][0]["minNotional"]
    )
