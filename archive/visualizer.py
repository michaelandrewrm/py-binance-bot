...
def plot_equity_curve_with_trades(
    equity_series, trades, title="Grid Strategy Equity Curve"
):
    ...
    if isinstance(trades, pd.DataFrame):
        ...
        plt.scatter(
            buy_trades["time"],
            equity_series.loc[buy_trades["time"]],
            color="green",
            marker="^",
            label="Buy",
            zorder=10,
        ...
        plt.scatter(
            sell_trades["time"],
            equity_series.loc[sell_trades["time"]],
            color="red",
            marker="v",
            label="Sell",
            zorder=10,
...
def plot_candlestick_with_trades(df, buy_orders, sell_orders, predicted_price):
    ...
    buys = (
        pd.DataFrame(buy_orders, columns=["Time", "Price"])
        if buy_orders
        else pd.DataFrame(columns=["Time", "Price"])
    )
    sells = (
        pd.DataFrame(sell_orders, columns=["Time", "Price"])
        if sell_orders
        else pd.DataFrame(columns=["Time", "Price"])
    )
    ...
    mpf.plot(
        ...
        volume=True,
        style="charles",
        title="Candlestick with Grid Trades and Prediction",
    )
