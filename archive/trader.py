...
from archived.binance_client import get_symbol_info, get_available_balance, get_current_price
...
class Trader:
    ...
    def adjust_grid_for_balance(self):
        base_asset, quote_asset = get_symbol_info(client=self.client, symbol=self.symbol)
        ...
        safe_levels = max(2, min(self.num_levels, max_buys + len(sell_prices) + 1, max_sells + len(buy_prices) + 1))
    ...
    def init_position_and_entry(self):
        ...
        self.logger.info(f"Initial spot position: {balance} {base_asset} at {self.avg_entry}")
    ...
    def initialize_grid(self, force=False):
        ...
        suggested_range = max(current_price * 0.01, min_grid_range * current_price * min_grid_levels, min_total_width)
        
        min_allowed = current_price * (1 - max_grid_range)
        max_allowed = current_price * (1 + max_grid_range)

        self.lower_price, self.upper_price = sorted([self.lower_price, self.upper_price])
        self.lower_price = max(min_allowed, min(max_allowed, self.lower_price))
        self.upper_price = max(self.lower_price + suggested_range, min(max_allowed, self.upper_price))
        ...
        if not grid_ok:
            ...
            self.logger.error(
                msg + f" — SUGGESTED: lower={suggested_lower}, upper={suggested_upper} (centered, width={suggested_range:.6f}, min step={min_step:.6f})"
            )
            if force:
                ...
                raise ValueError(msg + f" — SUGGESTED: lower={suggested_lower}, upper={suggested_upper}")
    ...
    def update_position_and_pnl(
        self, side, price, qty,
        commission, commission_asset, base_asset, quote_asset
    ):
        fee_in_quote = 0.0
        fee_in_base = 0.0
    ...
    def check_and_log_trades(self):
        ...
        for f in new_fills:
            ...
            realized_pnl = self.update_position_and_pnl(
                side, price, qty, commission,
                commission_asset, base_asset, quote_asset
            )
                ...
                try:
                    ...
                    if self.grid_trigger_price is not None and not self.grid_active:
                        if current_price >= self.grid_trigger_price:
                            self.logger.info(f"Grid trigger price hit: {current_price} >= {self.grid_trigger_price}. Activating grid.")
                            self.grid_active = True
                            self.sync_orders()
                            self.logger.info(f"Grid activated at {current_price}.")
                        else:
                            self.logger.info(f"Waiting for grid trigger. Current: {current_price}, Trigger: {self.grid_trigger_price}")
                            time.sleep(self.poll_interval)
                            continue  # Wait for trigger

                    # 2. STOP LOSS
                    if self.stop_loss_price is not None and self.grid_active and current_price <= self.stop_loss_price:
                        self.logger.warning(f"Stop loss triggered at {current_price} <= {self.stop_loss_price}. Cancelling grid.")
                        for oid in self.grid_orders.values():
                            self.cancel_order(oid)
                        self.grid_orders = {}
                        ...
                        if self.sell_all_on_stop:
                            ...
                            if base_balance > 0:
                                try:
                                    ...
                                    self.logger.info(f"Sold all {base_balance} {base_asset} at market on stop loss.")
                                except Exception as e:
                                    self.logger.error(f"Failed to sell all base at stop loss: {e}")
                        time.sleep(self.poll_interval)
                        continue

                    # 3. TAKE PROFIT
                    if self.take_profit_price is not None and self.grid_active and current_price >= self.take_profit_price:
                        self.logger.info(f"Take profit triggered at {current_price} >= {self.take_profit_price}. Cancelling grid.")
                        for oid in self.grid_orders.values():
                            self.cancel_order(oid)
                        self.grid_orders = {}
                        self.grid_active = False
                        if self.sell_all_on_stop:
                            ...
                            if base_balance > 0:
                                try:
                                    ...
                                    self.logger.info(f"Sold all {base_balance} {base_asset} at market on take profit.")
                                except Exception as e:
                                    self.logger.error(f"Failed to sell all base at take profit: {e}")
                        time.sleep(self.poll_interval)
                        continue

                    # 4. TRAILING UP
                    if self.trailing_up and self.grid_active and current_price > self.upper_price:
                        grid_width = self.upper_price - self.lower_price
                        self.logger.info(f"Price broke above upper grid ({current_price} > {self.upper_price}), trailing up grid.")
