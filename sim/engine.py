...
class BacktestEngine:
    ...
    async def run_backtest(
        self, strategy: TradingStrategy, market_data: List[KlineData]
    ) -> Dict:
        """
    ...
    def _generate_events(self, market_data: List[KlineData]):
        ...
        while current_time <= self.config.end_date:
            event = SimulationEvent(
                timestamp=current_time, event_type=EventType.TIMER, data={}
            )
            self.events.append(event)
    ...
    async def _process_event(self, event: SimulationEvent):
        ...
        try:
            ...
            logger.error(
                f"Error processing event {event.event_type} at {event.timestamp}: {e}"
            )
    ...
    async def _place_order(self, order: Order):
        ...
        try:
            ...
            if symbol_info:
                ...
                if not ensure_min_notional(
                    order.price, order.quantity, symbol_info.min_notional
                ):
                    logger.warning(
...
async def run_grid_strategy_backtest(
    symbol: str,
