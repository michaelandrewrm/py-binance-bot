...
class PerformanceEvaluator:
    ...
    def _calculate_returns(
        ...
        if years > 0 and final_value > 0:
            annualized_return = (final_value / initial_capital) ** (
                1 / float(years)
            ) - 1
    ...
    def _calculate_trade_metrics(self, trades: List[TradeRecord]) -> Dict:
        ...
        win_rate = (
            Decimal(str(win_count / total_trades)) if total_trades > 0 else Decimal("0")
        )
        ...
        average_win = (
            total_wins / len(winning_trades) if winning_trades else Decimal("0")
        )
        average_loss = (
            total_losses / len(losing_trades) if losing_trades else Decimal("0")
        )
    ...
    def _calculate_sortino_ratio(self, returns: Dict) -> Decimal:
        ...
        return self._calculate_sharpe_ratio(
            returns, returns["annualized_return"] * Decimal("0.2")
        )
    ...
    def _calculate_turnover(
        self, trades: List[TradeRecord], equity_curve: List[EquityCurvePoint]
    ) -> Decimal:
        ...
        average_capital = sum(point.net_worth for point in equity_curve) / len(
            equity_curve
        )
    ...
    def _calculate_var(self, returns: Dict, confidence: float) -> Decimal:
        ...
        z_score = (
            Decimal("1.645") if confidence == 0.95 else Decimal("2.326")
        )  # 95% or 99%
...
class BenchmarkComparison:
    ...
    def __init__(
        self,
        strategy_equity: List[EquityCurvePoint],
    ...
    def calculate_alpha_beta(self) -> Tuple[Decimal, Decimal]:
        ...
        for i in range(1, len(self.strategy_equity)):
            strat_ret = (
                self.strategy_equity[i].net_worth
                / self.strategy_equity[i - 1].net_worth
                - 1
            )
            bench_ret = (
                self.benchmark_equity[i].net_worth
                / self.benchmark_equity[i - 1].net_worth
                - 1
            )
        ...
        beta = (
            Decimal(str(covariance / benchmark_variance))
...
class RiskAnalyzer:
    ...
    def calculate_risk_metrics(equity_curve: List[EquityCurvePoint]) -> Dict:
        ...
        skewness = float(
            np.mean(
                ((returns_array - np.mean(returns_array)) / np.std(returns_array)) ** 3
            )
        )
        kurtosis = (
            float(
        ...
        return {
            ...
            "tail_ratio": Decimal(
                str(
                    abs(np.percentile(returns_array, 95))
...
def quick_performance_summary(
    ...
    profitable_trades = sum(
        1 for trade in trades if (trade.realized_pnl or Decimal("0")) > 0
    )
