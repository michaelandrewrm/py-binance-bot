...
from .schema import (
    KlineData, TradeData, TickerData, DepthData, TimeFrame,
    klines_from_binance, trades_from_binance, depth_from_binance
)
...
class DataValidator:
    ...
    def validate_klines(klines: List[KlineData]) -> Tuple[bool, List[str]]:
        ...
        for i, kline in enumerate(klines[1:], start=1):
            ...
            if not (kline.low_price <= kline.open_price <= kline.high_price):
                errors.append(f"Invalid OHLC relationship at index {i}: open {kline.open_price} not within range [{kline.low_price}, {kline.high_price}]")
            
            if not (kline.low_price <= kline.close_price <= kline.high_price):
                errors.append(f"Invalid OHLC relationship at index {i}: close {kline.close_price} not within range [{kline.low_price}, {kline.high_price}]")
            
            # Check for negative values
            if any(val < 0 for val in [kline.open_price, kline.high_price, kline.low_price, kline.close_price, kline.volume]):
                errors.append(f"Negative price or volume at index {i}")
            
            # Check for non-positive prices (should be > 0)
            if any(val <= 0 for val in [kline.open_price, kline.high_price, kline.low_price, kline.close_price]):
        ...
        for i in range(1, len(klines)):
            ...
            if abs((klines[i].open_time - expected_time).total_seconds()) > 60:  # 1 minute tolerance
    ...
    def validate_trades(trades: List[TradeData]) -> Tuple[bool, List[str]]:
        ...
        for i, trade in enumerate(trades):
            ...
            if abs(trade.quote_quantity - expected_quote) / expected_quote > 0.01:  # 1% tolerance
...
class ParquetCache:
    ...
    def _get_cache_path(self, data_type: str, symbol: str, timeframe: str = None, 
    ...
    def _is_cache_valid(self, cache_path: Path) -> bool:
        ...
        # Check age
        file_age = datetime.now(timezone.utc) - datetime.fromtimestamp(cache_path.stat().st_mtime, tz=timezone.utc)
        max_age = timedelta(hours=self.config.max_cache_age_hours)
        
        return file_age < max_age
    
    def save_klines(self, klines: List[KlineData], symbol: str, timeframe: TimeFrame,
                   start_date: datetime = None, end_date: datetime = None) -> Path:
        ...
        decimal_cols = ['open_price', 'high_price', 'low_price', 'close_price', 
        ...
        cache_path = self._get_cache_path("klines", symbol, timeframe.value, start_str, end_str)
    ...
    def load_klines(self, symbol: str, timeframe: TimeFrame,
                   start_date: datetime = None, end_date: datetime = None) -> Optional[List[KlineData]]:
    ...
    def clear_cache(self, older_than_hours: int = None):
        ...
        logger.info(f"Cleared {deleted_count} cache files older than {older_than_hours} hours")
...
class DataLoader:
    ...
    def __init__(self, cache_config: CacheConfig = None, enable_validation: bool = True):
        self.cache_config = cache_config or CacheConfig()
        self.cache = ParquetCache(self.cache_config)
        self.enable_validation = enable_validation
        self.validator = DataValidator()
    
    async def load_klines(self, symbol: str, timeframe: TimeFrame, 
                         start_date: datetime, end_date: datetime,
                         force_refresh: bool = False) -> List[KlineData]:
        """
        ...
        if not force_refresh:
            cached_klines = self.cache.load_klines(symbol, timeframe, start_date, end_date)
        ...
        logger.info(f"Loading klines from source: {symbol} {timeframe.value} {start_date} to {end_date}")
    ...
    async def load_trades(self, symbol: str, start_date: datetime, end_date: datetime,
                         force_refresh: bool = False) -> List[TradeData]:
...
def create_data_loader(cache_dir: str = None, 
                      max_cache_age_hours: int = None,
                      use_unified_config: bool = True) -> DataLoader:
...
async def load_historical_data(loader: DataLoader, symbol: str, timeframe: TimeFrame,
                              days_back: int = 30) -> List[KlineData]:
