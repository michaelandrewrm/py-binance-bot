...
class DashboardData:
    ...
    async def get_portfolio_summary(self) -> Dict:
        ...
        pnl_24h = sum(float(trade.pnl or 0) for trade in recent_trades 
                     if trade.timestamp >= now - timedelta(hours=24))
        pnl_7d = sum(float(trade.pnl or 0) for trade in recent_trades 
        ...
        return [
            {
                ...
                'data_age_hours': (datetime.now(timezone.utc) - row['end_date']).total_seconds() / 3600 
...
class SimpleDashboard:
    ...
    async def display_recent_trades(self, limit: int = 10):
        ...
        print("-" * 80)
        print(f"{'Time':<20} {'Symbol':<10} {'Side':<5} {'Qty':<12} {'Price':<12} {'P&L':<10}")
        print("-" * 80)
        ...
        for trade in trades:
            ...
            print(f"{timestamp:<20} {trade['symbol']:<10} {trade['side']:<5} "
    ...
    async def display_strategy_performance(self):
        ...
        print("-" * 70)
        print(f"{'Strategy':<15} {'Trades':<8} {'Win Rate':<10} {'Total P&L':<12} {'Avg P&L':<10}")
        print("-" * 70)
        
        for strategy in strategies:
            print(f"{strategy['strategy']:<15} {strategy['total_trades']:<8} "
    ...
    async def display_data_status(self):
        ...
        print(f"\nMARKET DATA STATUS")
        print("-" * 80)
        print(f"{'Symbol':<10} {'Interval':<10} {'Records':<10} {'Last Update':<20} {'Age (h)':<8}")
        print("-" * 80)
        
        for status in data_status:
            last_update = datetime.fromisoformat(status['end_date']).strftime('%m-%d %H:%M') if status['end_date'] else 'N/A'
            age = f"{status['data_age_hours']:.1f}" if status['data_age_hours'] else 'N/A'
            
            print(f"{status['symbol']:<10} {status['interval']:<10} {status['record_count']:<10} "
