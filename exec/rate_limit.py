    ...
    async def acquire(self, weight: int = 1, retries: int = 3) -> bool:
        ...
        for attempt in range(retries + 1):
            ...
            if attempt < retries:
                ...
                logger.debug(f"Rate limited, waiting {wait_time:.2f}s (attempt {attempt + 1}/{retries + 1})")
...
class AdaptiveRateLimiter(RateLimiter):
    ...
    def record_error(self, error_type: str):
        ...
        error_rate = len(self.recent_errors) / self.error_window * 60  # errors per minute
        
        if error_rate > 5:  # More than 5 errors per minute
            self.adaptive_factor = max(0.5, self.adaptive_factor * 0.9)
            logger.warning(f"High error rate detected, reducing limits by factor {self.adaptive_factor}")
        elif error_rate < 1:  # Less than 1 error per minute
            self.adaptive_factor = min(1.0, self.adaptive_factor * 1.05)
            logger.info(f"Low error rate, increasing limits by factor {self.adaptive_factor}")
