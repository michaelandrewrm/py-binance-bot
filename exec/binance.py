...
class BinanceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(f"Binance client configured for {'testnet' if self.testnet else 'mainnet'} with API key {mask_api_key(self.api_key)}")
    ...
    def from_unified_config(cls, config: Optional[Any] = None) -> 'BinanceConfig':
        ...
        if not _unified_config_available:
            raise ImportError("Unified config system not available. Install required dependencies.")
        ...
        return cls(
            ...
            base_url=config.binance.base_url or ("https://testnet.binance.vision" if config.binance.testnet else "https://api.binance.com"),
...
class BinanceError(Exception):
    ...
    def __init__(self, message: str, code: Optional[int] = None, response: Optional[Dict] = None) -> None:
...
class BinanceClient:
    ...
    async def _make_request_with_retry(self, method: str, endpoint: str, params: Dict = None, 
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                rate_limit_acquired = await self.rate_limiter.acquire(weight=weight, retries=2)
                if not rate_limit_acquired:
                    raise BinanceRateLimitError("Rate limit acquisition failed after retries")
                
                return await self._make_request_single(method, endpoint, params, signed, weight)
                
            except BinanceRateLimitError as e:
                last_exception = e
                if attempt < self.config.max_retries:
                    delay = self._calculate_retry_delay(attempt)
                    logger.warning(f"Rate limit hit, retrying in {delay:.2f}s (attempt {attempt + 1}/{self.config.max_retries + 1})")
                    await asyncio.sleep(delay)
                    continue
                else:
                    logger.error(f"Rate limit retries exhausted after {self.config.max_retries} attempts")
                    raise
                ...
                if attempt < self.config.max_retries:
                    delay = self._calculate_retry_delay(attempt)
                    logger.warning(f"Network error, retrying in {delay:.2f}s (attempt {attempt + 1}/{self.config.max_retries + 1}): {e}")
                    await asyncio.sleep(delay)
                    continue
                else:
                    logger.error(f"Network retries exhausted after {self.config.max_retries} attempts")
                    raise last_exception
                    
            except BinanceError as e:
                # For API errors, only retry on specific error codes
                if e.code in [429, 418, -1021] and attempt < self.config.max_retries:  # Rate limit, banned, timestamp errors
                    delay = self._calculate_retry_delay(attempt)
                    logger.warning(f"API error {e.code}, retrying in {delay:.2f}s (attempt {attempt + 1}/{self.config.max_retries + 1}): {e}")
                    await asyncio.sleep(delay)
                    continue
                else:
                ...
                logger.error(f"Unexpected error in request (attempt {attempt + 1}): {e}")
                if attempt >= self.config.max_retries:
                    raise last_exception
    ...
    async def _make_request_single(self, method: str, endpoint: str, params: Dict = None, 
                                 signed: bool = False, weight: int = 1) -> Dict:
        ...
        try:
            async with self.session.request(method, url, params=params, headers=headers) as response:
                ...
                if response.status == 429:
                    raise BinanceRateLimitError("Rate limit exceeded", response.status, data)
    ...
    async def _make_request(self, method: str, endpoint: str, params: Dict = None, 
        ...
        return await self._make_request_with_retry(method, endpoint, params, signed, weight)
    ...
    async def get_account_info(self) -> Dict:
        ...
        return await self._make_request("GET", "/api/v3/account", signed=True, weight=10)
    ...
    async def place_order(self, symbol: str, side: str, order_type: str,
        ...
        return await self._make_request("POST", "/api/v3/order", params, signed=True, weight=1)
    ...
    async def get_order(self, symbol: str, order_id: int = None,
        ...
        return await self._make_request("GET", "/api/v3/order", params, signed=True, weight=2)
    
    async def get_open_orders(self, symbol: str = None) -> List[Dict]:
        """Get all open orders"""
        params = {"symbol": symbol} if symbol else {}
        weight = 3 if symbol else 40
        return await self._make_request("GET", "/api/v3/openOrders", params, signed=True, weight=weight)
    
    async def cancel_all_orders(self, symbol: str) -> List[Dict]:
        """Cancel all open orders for a symbol"""
        params = {"symbol": symbol}
        return await self._make_request("DELETE", "/api/v3/openOrders", params, signed=True, weight=1)
    
    async def get_order_history(self, symbol: str, limit: int = 500) -> List[Dict]:
        """Get order history"""
        params = {"symbol": symbol, "limit": limit}
        return await self._make_request("GET", "/api/v3/allOrders", params, signed=True, weight=10)
    
    async def get_trades(self, symbol: str, limit: int = 500) -> List[Dict]:
        """Get trade history"""
        params = {"symbol": symbol, "limit": limit}
        return await self._make_request("GET", "/api/v3/myTrades", params, signed=True, weight=10)
    ...
    def convert_order_to_internal(self, binance_order: Dict) -> Order:
        ...
        return Order(
            ...
            filled_price=Decimal(binance_order.get("price", "0")),  # Avg price not always available
...
async def create_binance_client(api_key: str = None, api_secret: str = None, testnet: bool = None) -> BinanceClient:
    ...
    if api_key is None and api_secret is None and testnet is None:
        if _unified_config_available:
            ...
            raise ValueError("No API credentials provided and unified config not available")
    else:
        # Use provided parameters or fallback to unified config for missing ones
        if _unified_config_available and (api_key is None or api_secret is None or testnet is None):
            ...
            testnet = testnet if testnet is not None else unified_settings.binance.testnet
