...
class LinearSlippageModel(SlippageModel):
    ...
    def calculate_slippage(self, order: Order, market_data: MarketData) -> Decimal:
        ...
        size_factor = min(
            order.quantity * order.price / 10000, 1.0
        )  # Cap at 100% impact
...
class PaperBroker:
    ...
    def __init__(
        ...
        self.balances = {
            asset: Balance(asset, balance, Decimal("0"))
    ...
    async def _fill_order(self, order: Order, fill_price: Decimal):
        ...
        slippage = self.slippage_model.calculate_slippage(
            order, self.market_data[order.symbol]
        )
        final_price = fill_price * (1 + slippage)

        # Calculate fees
        sizing_calc = self.sizing_calculators[order.symbol]
        fee_info = sizing_calc.calculate_fees(
            final_price, order.quantity, is_maker=True
        )
        ...
        logger.info(
            f"Filled order: {order.quantity} {order.symbol} @ {final_price} (slippage: {slippage:.4%})"
        )
    ...
    def _check_balance(
        ...
        if side == OrderSide.BUY:
            ...
            available = self.balances.get(
                quote_asset, Balance(quote_asset, Decimal("0"), Decimal("0"))
            ).free
            return available >= required
        else:
            # Need base asset to sell
            available = self.balances.get(
                base_asset, Balance(base_asset, Decimal("0"), Decimal("0"))
            ).free
    ...
    def _execute_trade(self, order: Order, fill_price: Decimal, fee: Decimal):
        ...
        if order.side == OrderSide.BUY:
            ...
            if base_asset not in self.balances:
                self.balances[base_asset] = Balance(
            ...
            if quote_asset not in self.balances:
                self.balances[quote_asset] = Balance(
                    quote_asset, Decimal("0"), Decimal("0")
                )
    ...
    def get_account_info(self) -> Dict:
        ...
        return {
            ...
            "total_wallet_balance": str(
                sum(b.total for b in self.balances.values() if b.asset == "USDC")
            ),
    ...
    def get_order_status(self, order_id: str) -> Optional[Dict]:
        ...
        return {
            ...
            "update_time": (
                int(order.update_time.timestamp() * 1000) if order.update_time else None
            ),
        }
