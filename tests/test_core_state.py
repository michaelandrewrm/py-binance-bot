...
class TestBotState:
    ...
    def test_update_position_existing_position_increase(self):
        ...
        expected_avg_price = (Decimal('0.001') * Decimal('50000') + Decimal('0.002') * Decimal('52000')) / Decimal('0.003')
    ...
    def test_update_order_active(self):
        ...
        # Update order
        state.update_order("12345", status=OrderStatus.PARTIALLY_FILLED, filled_quantity=Decimal('0.0005'))
    ...
    def test_update_order_move_to_history(self):
        ...
        # Fill order
        state.update_order("12345", status=OrderStatus.FILLED, filled_quantity=Decimal('0.001'))
    ...
    def test_get_total_portfolio_value(self):
        ...
        expected = Decimal('1100') + (Decimal('0.025') * Decimal('50000')) + (Decimal('1.7') * Decimal('3000'))
...
class TestComplexScenarios:
    ...
    def test_full_trading_session_simulation(self):
        ...
        # Fill buy order
        state.update_order("buy_1", status=OrderStatus.FILLED, filled_quantity=Decimal('0.002'))
        state.update_position("BTCUSDC", Decimal('0.002'), Decimal('49000'))
        state.update_balance("USDC", Decimal('10000') - Decimal('98'), Decimal('0'))  # 0.002 * 49000
        state.update_balance("BTC", Decimal('0.002'), Decimal('0'))
        
        # Partial fill sell order
        state.update_order("sell_1", status=OrderStatus.PARTIALLY_FILLED, filled_quantity=Decimal('0.0005'))
    ...
    def test_position_averaging_scenario(self):
        ...
        expected_avg = (Decimal('50') + Decimal('96') + Decimal('52')) / Decimal('0.004')
    ...
    def test_serialization_roundtrip_with_complex_data(self):
        ...
        try:
            ...
            assert loaded_state.balances["USDC"]["free"] == "1000.123456"  # JSON stores as string
