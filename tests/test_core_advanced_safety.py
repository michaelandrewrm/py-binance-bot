...
from core.advanced_safety import (
    RiskLevel, MarketRegime, AlertType,
    RiskMetrics, MarketConditions, SafetyAlert,
    AdvancedSafetyManager, advanced_safety_manager
)
...
class TestRiskLevel:
    ...
    def test_risk_level_ordering(self):
        ...
        levels = [RiskLevel.LOW, RiskLevel.MODERATE, RiskLevel.HIGH, RiskLevel.EXTREME, RiskLevel.CRITICAL]
        assert len(levels) == 5
...
class TestMarketRegime:
    ...
    def test_market_regime_complete(self):
        ...
        regimes = [MarketRegime.CALM, MarketRegime.NORMAL, MarketRegime.VOLATILE, MarketRegime.EXTREME, MarketRegime.CRISIS]
...
class TestAdvancedSafetyManager:
    ...
    def test_calculate_risk_metrics_new_session(self, safety_manager):
        ...
        metrics = safety_manager._calculate_risk_metrics(session_id, current_pnl, position_size)
...
class TestComplexScenarios:
    ...
    def test_market_regime_transitions(self, safety_manager):
        ...
        for i in range(20):
            ...
            safety_manager.update_market_data(symbol, price, Decimal('100'), 
        ...
        if conditions:
            ...
            assert conditions.regime != initial_regime or conditions.volatility_percentile > 0.5
    ...
    def test_risk_metrics_calculation_edge_cases(self, safety_manager):
        ...
        metrics = safety_manager._calculate_risk_metrics(session_id, Decimal('0'), Decimal('0'))
        assert metrics.current_drawdown == Decimal('0')
        assert metrics.position_exposure == Decimal('0')
        
        # Test with positive PnL
        metrics = safety_manager._calculate_risk_metrics(session_id, Decimal('500'), Decimal('1000'))
        assert metrics.current_drawdown == Decimal('0')  # No drawdown with profit
        
        # Test with extreme negative PnL
        metrics = safety_manager._calculate_risk_metrics(session_id, Decimal('-5000'), Decimal('1000'))
    ...
    def test_alert_categorization_and_filtering(self, safety_manager):
        ...
        for alert_type, category, title, message in alert_configs:
            safety_manager._create_alert(alert_type, category, title, message, session_id)
        
        # Test filtering by session
        session_alerts = [a for a in safety_manager.alerts if a.session_id == session_id]
        assert len(session_alerts) == 5
        
        # Test filtering by type
        critical_alerts = [a for a in safety_manager.alerts if a.alert_type == AlertType.CRITICAL]
