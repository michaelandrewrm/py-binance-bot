...
from exec.rate_limit import (
    ...
    RateLimiter,
    AdaptiveRateLimiter,
    create_conservative_rate_limiter,
...
class TestExecBinanceClient:
    ...
    async def test_binance_client_context_manager(self, binance_client):
        ...
        with patch.object(binance_client, "start_session") as mock_start, patch.object(
            binance_client, "close_session"
        ) as mock_close:
    ...
    async def test_start_session(self, binance_client):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class, patch.object(
            binance_client, "_sync_server_time"
        ) as mock_sync:
    ...
    async def test_make_request_single_success(self, binance_client):
        ...
        result = await binance_client._make_request_single(
    ...
    async def test_make_request_with_retry_success_after_retry(self, binance_client):
        ...
        with patch.object(binance_client, "_make_request_single") as mock_single:
            ...
            with patch("asyncio.sleep"):
                result = await binance_client._make_request_with_retry(
                    "GET", "/api/v3/test"
                )
    ...
    async def test_cancel_order_with_order_id(self, binance_client):
        ...
        with patch.object(binance_client, "_make_request") as mock_request:
            ...
            mock_request.assert_called_once_with(
                "DELETE", "/api/v3/order", expected_params, signed=True, weight=1
            )
...
class TestExecPaperBrokerSlippage:
    ...
    def test_linear_slippage_model_creation(self):
        ...
        model = LinearSlippageModel(
            base_slippage_bps=Decimal("5"), size_impact_factor=Decimal("0.2")
        )
...
class TestExecPaperBroker:
    ...
    def fee_config(self):
        ...
        return FeeConfig(maker_fee=Decimal("0.001"), taker_fee=Decimal("0.001"))

    @pytest.fixture
    def paper_broker(self, exchange_info, fee_config):
        """Fixture for PaperBroker"""
        initial_balances = {"BTC": Decimal("10"), "USDT": Decimal("100000")}
    ...
    def test_check_balance_sufficient(self, paper_broker):
        ...
        result = paper_broker._check_balance(
            "BTCUSDT", OrderSide.BUY, Decimal("1.0"), Decimal("50000")
        )
        assert result is True

        result = paper_broker._check_balance(
            "BTCUSDT", OrderSide.SELL, Decimal("5.0"), Decimal("50000")
        )
        assert result is True
    ...
    def test_check_balance_insufficient(self, paper_broker):
        ...
        result = paper_broker._check_balance(
            "BTCUSDT", OrderSide.BUY, Decimal("10.0"), Decimal("50000")
        )
        assert result is False

        result = paper_broker._check_balance(
            "BTCUSDT", OrderSide.SELL, Decimal("20.0"), Decimal("50000")
        )
        assert result is False
...
class TestExecRateLimitConfig:
    ...
    def test_rate_limit_config_custom(self):
        ...
        config = RateLimitConfig(
            requests_per_minute=600, weight_per_minute=3000, order_limit_per_second=5
        )
...
class TestExecRateLimiter:
    ...
    def test_reset_buckets(self, rate_limiter):
        ...
        assert (
            rate_limiter.request_bucket.tokens == rate_limiter.request_bucket.capacity
        )
