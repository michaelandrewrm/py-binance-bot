...
from core.grid_engine import GridDirection, GridLevel, GridConfig, GridEngine
...
class TestGridLevel:
    ...
    def test_grid_level_creation(self):
        ...
        level = GridLevel(price=Decimal("50000"), level_index=1)
    ...
    def test_grid_level_decimal_precision(self):
        ...
        level = GridLevel(price=Decimal("50000.123456"), level_index=0)
    ...
    def test_grid_level_negative_index(self):
        ...
        level = GridLevel(price=Decimal("45000"), level_index=-5)
...
class TestGridEngine:
    ...
    def test_on_fill_edge_level_buy(self):
        ...
        new_orders = self.engine.on_fill(
            highest_buy_index, "edge_buy", Decimal("47500")
        )
    ...
    def test_on_fill_edge_level_sell(self):
        ...
        new_orders = self.engine.on_fill(
            highest_sell_index, "edge_sell", Decimal("52500")
        )
    ...
    def test_rebalance_grid_center_price_update(self):
        ...
        for level in self.engine.grid_levels.values():
            if level.level_index > 0:
                expected_price = new_center_price + (
                    self.config.grid_spacing * level.level_index
                )
            else:
                expected_price = new_center_price + (
                    self.config.grid_spacing * level.level_index
    ...
    def test_get_grid_status_with_activity(self):
        ...
        expected_position = (
            self.config.order_amount - self.config.order_amount
        )  # buy - sell = 0
...
class TestGridEngineIntegration:
    ...
    def test_small_grid_spacing(self):
        ...
        for level in levels:
            ...
            expected_price = config.center_price + (
                config.grid_spacing * level.level_index
            )
            assert level.price == expected_price
    ...
    def test_large_grid_spacing(self):
        ...
        expected_range = config.grid_spacing * (
            config.num_levels_up + config.num_levels_down
        )
        actual_range = max_price - min_price

        assert actual_range == expected_range
        assert min_price == config.center_price - (
            config.grid_spacing * config.num_levels_down
        )
        assert max_price == config.center_price + (
            config.grid_spacing * config.num_levels_up
        )
    ...
    def test_grid_rebalancing_scenarios(self):
        ...
        center_level_prices = [
            l.price for l in new_levels if l.level_index == 1 or l.level_index == -1
        ]

        # Should be evenly spaced around new center
        for level in new_levels:
            expected_price = large_move_price + (
                config.grid_spacing * level.level_index
            )
            assert level.price == expected_price
    ...
    def test_grid_level_side_assignment(self):
        ...
        sell_prices = [
            l.price for l in sorted(sell_levels, key=lambda x: x.level_index)
        ]
    ...
    def test_position_tracking_accuracy(self):
        ...
        for level_idx, side, delta in fills:
            ...
            assert (
                engine.current_position == expected_position
    ...
    def test_edge_case_handling(self):
        ...
        assert all(
            key in status for key in ["total_levels", "filled_levels", "active_orders"]
        )
