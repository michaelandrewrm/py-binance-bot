...
class TestDataValidator:
    ...
    def test_validate_ohlc_relationships(self):
        ...
        kline2 = self._create_sample_kline(
            datetime(2023, 1, 1, 0, 5, tzinfo=timezone.utc)
        )
    ...
    def test_validate_negative_values(self):
        ...
        kline1 = self._create_sample_kline(
            datetime(2023, 1, 1, 0, 0, tzinfo=timezone.utc), volume=Decimal("-100")
        )
        kline2 = self._create_sample_kline(
            datetime(2023, 1, 1, 0, 5, tzinfo=timezone.utc)
        )
    ...
    def test_validate_time_gaps(self):
        ...
        klines = [
            self._create_sample_kline(datetime(2023, 1, 1, 0, 0, tzinfo=timezone.utc)),
            self._create_sample_kline(
                datetime(2023, 1, 1, 0, 15, tzinfo=timezone.utc)
            ),  # 15min gap for 5min timeframe
        ]
        is_valid, errors = DataValidator.validate_klines(klines)
        assert is_valid is False
...
class TestParquetCache:
    ...
    def setup_method(self):
        ...
        self.cache_config = CacheConfig(cache_dir=self.temp_dir, max_cache_age_hours=1)
    ...
    def test_cache_expiry(self):
        ...
        short_cache_config = CacheConfig(
            cache_dir=self.temp_dir, max_cache_age_hours=0  # Immediate expiry
        )
...
class TestDataLoader:
    ...
    def test_create_data_loader_function(self):
        ...
        loader = create_data_loader(
            cache_dir=self.temp_dir, max_cache_age_hours=2, use_unified_config=False
        )
