...
class TestTradingSession:
    ...
    def test_is_active_method(self):
        ...
        params = GridParameters("BTCUSDC", Decimal("48000"), Decimal("52000"), 20, Decimal("50"))
    ...
    def test_is_time_expired_with_limit(self):
        ...
        session = TradingSession("test-1", "BTCUSDC", params)
        session.duration_limit = timedelta(hours=1)
        session.start_time = datetime.now(timezone.utc) - timedelta(hours=2)  # Started 2 hours ago
    ...
    def test_is_time_expired_with_paused_time(self):
        ...
        session.total_paused_duration = timedelta(hours=1, minutes=30)  # Paused for 1.5 hours
    ...
    def test_get_elapsed_time_with_start(self):
        ...
        elapsed = session.get_elapsed_time()
        assert abs(elapsed - timedelta(hours=1)) < timedelta(seconds=1)  # Allow small tolerance
...
class TestAsyncFunctions:
    ...
    async def test_get_ai_suggested_parameters_success(self):
        ...
        with patch('ui.cli.get_market_context') as mock_context, \
             patch('ui.cli.BayesianOptimizer') as mock_optimizer:
    ...
    async def test_get_ai_suggested_parameters_no_optimizer(self):
        ...
        with patch('ui.cli.get_market_context') as mock_context, \
             patch('ui.cli.BayesianOptimizer', side_effect=ImportError):
...
class TestCLICommands:
    ...
    def test_collect_manual_parameters_mocked_input(self):
        ...
        with patch('ui.cli.Prompt.ask') as mock_prompt, \
             patch('ui.cli.FloatPrompt.ask') as mock_float_prompt, \
             patch('ui.cli.IntPrompt.ask') as mock_int_prompt, \
             patch('ui.cli.Confirm.ask') as mock_confirm:
            
            # Mock user inputs
            mock_float_prompt.side_effect = [48000.0, 52000.0, 50.0]  # bounds, investment
    ...
    def test_confirm_ai_parameters_modify(self):
        ...
        with patch('ui.cli.Confirm.ask', return_value=False), \
             patch('ui.cli.collect_manual_parameters') as mock_collect:
