...
class TestDashboardData:
    ...
    async def test_get_portfolio_summary_with_trades(self):
        ...
        mock_trades = [
            Mock(
                pnl=Decimal('50.0'),
                timestamp=now - timedelta(hours=1)  # Within 24h
            ),
            Mock(
                pnl=Decimal('-20.0'),
            ...
            Mock(
                pnl=Decimal('75.0'),
                timestamp=now - timedelta(hours=12)  # Within 24h
            )
        ]
...
class TestUtilityFunctions:
    ...
    def test_run_streamlit_dashboard_success(self):
        ...
        with patch('ui.dashboard.create_streamlit_dashboard', return_value=True) as mock_create, \
    ...
    def test_run_streamlit_dashboard_fallback(self):
        ...
        with patch('ui.dashboard.create_streamlit_dashboard', return_value=False) as mock_create, \
             patch('ui.dashboard.run_simple_dashboard') as mock_run_simple, \
             patch('builtins.print') as mock_print:
