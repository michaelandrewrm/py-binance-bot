...
from core.baseline_heuristic import BaselineHeuristic, get_baseline_parameters
...
class TestBaselineHeuristic:
    ...
    def test_calculate_grid_parameters_normal_volatility(self):
        ...
        assert (
            result["reasoning"]["method"]
            == "ATR-based bounds with volatility-adjusted grid count"
        )
    ...
    def test_calculate_grid_parameters_high_volatility(self):
        ...
        result = self.heuristic.calculate_grid_parameters(
            symbol=symbol, current_price=current_price, atr=atr, volatility=volatility
        )
    ...
    def test_fallback_parameters_timestamp(self):
        ...
        result = self.heuristic._fallback_parameters(
            "TEST", Decimal("100"), Decimal("10")
        )
...
class TestIntegrationScenarios:
    ...
    def test_parameter_generation_workflow(self):
        ...
        for case in test_cases:
            ...
            result = get_baseline_parameters(
                case["symbol"], market_data, Decimal(str(case["investment"]))
            )
    ...
    def test_parameter_consistency(self):
        ...
        symbol = "CONSISTUSDC"
        market_data = {"current_price": 1000.0, "atr": 50.0, "volatility": 0.03}
    ...
    def test_volatility_impact_on_grid_count(self):
        ...
        base_params = {"symbol": "TESTUSDC", "current_price": 1000.0, "atr": 50.0}

        # Test low, medium, and high volatility
        low_vol_result = get_baseline_parameters(
            "TEST", {**base_params, "volatility": 0.01}
        )
        med_vol_result = get_baseline_parameters(
            "TEST", {**base_params, "volatility": 0.04}
        )
        high_vol_result = get_baseline_parameters(
            "TEST", {**base_params, "volatility": 0.08}
        )
