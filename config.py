...
class DatabaseConfig(BaseModel):
    ...
    max_overflow: int = Field(
        default=10, description="Maximum connection overflow", ge=0, le=100
    )
    ...
    def validate_database_url(cls, v: str) -> str:
        ...
        if not any(
            ...
            logger.warning(
                f"Unrecognized database URL scheme: {v.split('://')[0] if '://' in v else 'none'}"
            )
...
class BinanceConfig(BaseModel):
    ...
    api_secret: Optional[str] = Field(
        default=None, description="Binance API secret (secret)"
    )
    testnet: bool = Field(default=True, description="Use Binance testnet")
    base_url: Optional[str] = Field(default=None, description="Custom API base URL")
    rate_limit_requests_per_minute: int = Field(
        default=1200, description="API rate limit", ge=1, le=6000
    )
    rate_limit_orders_per_second: int = Field(
        default=10, description="Order rate limit", ge=1, le=100
    )
...
class LoggingConfig(BaseModel):
    ...
    max_file_size_mb: int = Field(
        default=10, description="Maximum log file size in MB", ge=1, le=1000
    )
    backup_count: int = Field(
        default=5, description="Number of backup log files", ge=0, le=50
    )
    format: str = Field(
...
class GridConfig(BaseModel):
    ...
    n_grids: int = Field(default=12, description="Number of grid levels", ge=2, le=100)
    ...
    upper_price: Optional[Decimal] = Field(
        default=None, description="Upper price boundary"
    )
    lower_price: Optional[Decimal] = Field(
        default=None, description="Lower price boundary"
    )
...
class AIConfig(BaseModel):
    ...
    epochs: int = Field(default=30, description="Training epochs", ge=1, le=1000)
    batch_size: int = Field(
        default=32, description="Training batch size", ge=1, le=1024
    )
    ...
    early_stopping_patience: int = Field(
        default=5, description="Early stopping patience", ge=1, le=100
    )
    ...
    def validate_model_type(cls, v: str) -> str:
        ...
        if v not in VALID_MODEL_TYPES:
            raise ValueError(
                f"Invalid model type '{v}'. Must be one of: {VALID_MODEL_TYPES}"
            )
    ...
    def validate_batch_size(cls, v: int) -> int:
        ...
        if v & (v - 1) != 0:
            logger.warning(
                f"Batch size {v} is not a power of 2, which may impact performance"
            )
...
class CacheConfig(BaseModel):
    ...
    max_size_mb: int = Field(
        default=500, description="Maximum cache size in MB", ge=10, le=10000
    )
    ...
    def validate_cache_dir(cls, v: Path | str) -> Path:
        ...
        if str(v) in [".", "", "/"]:
            raise ValueError(
                "Cache directory cannot be current directory, empty, or root"
            )
        ...
        if any(str(abs_path).startswith(sys_dir) for sys_dir in system_dirs):
            raise ValueError(
                f"Cache directory cannot be in system directory: {abs_path}"
            )
...
class SecurityConfig(BaseModel):
    ...
    jwt_secret_key: Optional[str] = Field(
        default=None, description="JWT secret key (secret)"
    )
    encryption_key: Optional[str] = Field(
        default=None, description="Encryption key (secret)"
    )
    ...
    def is_secure(self) -> bool:
        ...
        return (
            self.jwt_secret_key is not None
            and len(self.jwt_secret_key) >= MIN_JWT_SECRET_LENGTH
        )
...
class NotificationConfig(BaseModel):
    ...
    telegram_bot_token: Optional[str] = Field(
        default=None, description="Telegram bot token (secret)"
    )
    telegram_chat_id: Optional[str] = Field(
        default=None, description="Telegram chat ID (secret)"
    )
    ...
    email_username: Optional[str] = Field(
        default=None, description="Email username (secret)"
    )
    email_password: Optional[str] = Field(
        default=None, description="Email password (secret)"
    )
    ...
    def has_telegram_config(self) -> bool:
        ...
        return (
            ...
            and len(self.telegram_bot_token.strip()) > 0
            and len(self.telegram_chat_id.strip()) > 0
        )
    ...
    def has_email_config(self) -> bool:
        ...
        return (
            self.email_enabled
            and self.email_smtp_server is not None
            and self.email_username is not None
            and self.email_password is not None
            and len(self.email_smtp_server.strip()) > 0
            and len(self.email_username.strip()) > 0
            and len(self.email_password.strip()) > 0
        )
...
class Settings(BaseSettings):
    ...
    def _load_yaml_config(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_mode & 0o077:  # group or other have any permissions
                logger.warning(
                    f"Configuration file {config_file} has permissions {oct(file_mode)}, should be 0o600 or more restrictive"
                )
            ...
            logger.error(
                f"Unexpected error loading configuration file {config_file}: {e}"
            )
    ...
    def _interpolate_env_vars(self, content: str) -> str:
        ...
        def replace_var(match):
            ...
            if len(var_name) > 100:  # Prevent extremely long variable names
                logger.warning(
                    f"Environment variable name too long: {var_name[:50]}..."
                )
    ...
    def _merge_configurations(
        self, yaml_config: Dict[str, Any], kwargs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_configuration_consistency(self) -> None:
        ...
        if (
            self.notifications.telegram_enabled
            and not self.notifications.has_telegram_config()
        ):
            logger.warning(
                "Telegram notifications enabled but configuration incomplete"
            )

        if (
            self.notifications.email_enabled
            and not self.notifications.has_email_config()
        ):
    ...
    def _validate_credentials(self) -> List[str]:
        ...
        if not self.is_development() and not self.binance.has_credentials():
            issues.append(
                "Binance API credentials are missing for non-development environment"
            )
    ...
    def _validate_security_config(self) -> List[str]:
        ...
        if self.is_production():
            if (
                ...
                and len(self.security.jwt_secret_key) < MIN_JWT_SECRET_LENGTH
            ):
                issues.append(
...
class ConfigurationManager:
    ...
    def get_settings(
        self, config_file: Optional[str] = None, reload: bool = False
    ) -> Settings:
        """
        ...
        should_reload = (
            ...
            or reload
            or (config_file is not None and config_file != self._config_file)
            or self._should_auto_reload(config_file or self._config_file)
        )
