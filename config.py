...
class DatabaseConfig(BaseModel):
    ...
    url: str = Field(
        default="sqlite:///trading_bot.db", description="Database connection URL"
    )
...
class BinanceConfig(BaseModel):
    ...
    rate_limit_requests_per_minute: int = Field(
        default=1200, description="API rate limit"
    )
    rate_limit_orders_per_second: int = Field(
        default=10, description="Order rate limit"
    )
...
class TradingConfig(BaseModel):
    ...
    max_position_size: Decimal = Field(
        default=Decimal("1000"), description="Maximum position size"
    )
    risk_per_trade: Decimal = Field(
        default=Decimal("0.02"), description="Risk percentage per trade"
    )
    fee_percentage: Decimal = Field(
        default=Decimal("0.001"), description="Trading fee percentage"
    )
...
class GridConfig(BaseModel):
    ...
    invest_per_grid: Decimal = Field(
        default=Decimal("50.0"), description="Investment per grid level"
    )
    grid_spacing_pct: Decimal = Field(
        default=Decimal("0.01"), description="Grid spacing percentage"
    )
...
class AIConfig(BaseModel):
    ...
    validation_split: Decimal = Field(
        default=Decimal("0.1"), description="Validation split ratio"
    )
...
class Settings(BaseSettings):
    ...
    def _load_yaml_config(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Loaded configuration from {config_file} for environment: {env}"
            )
    ...
    def validate_configuration(self) -> List[str]:
        ...
        if self.notifications.telegram_enabled:
            if (
                ...
                or not self.notifications.telegram_chat_id
            ):
