...
class FlattenConfig:
    ...
    market_order_threshold_pct: Decimal = Decimal('0.95')  # Use market if 95% of TWAP time elapsed
...
class PositionFlattener:
    ...
    async def flatten_position(self, symbol: str, quantity: Decimal) -> FlattenResult:
        ...
        try:
            ...
            slice_interval = self.config.twap_duration_minutes * 60 / self.config.twap_slice_count
            
            total_proceeds = Decimal('0')
            total_fees = Decimal('0')
            executed_quantity = Decimal('0')
            
            logger.info(f"Starting TWAP flatten for {symbol}: {abs_quantity} {side.value} over {self.config.twap_duration_minutes}m")
            ...
            for slice_num in range(self.config.twap_slice_count):
                ...
                if elapsed_pct >= self.config.market_order_threshold_pct:
                    logger.info(f"Switching to market order for remaining {remaining} (timeout threshold reached)")
                    proceeds, fees = await self._execute_market_order(symbol, side, remaining)
                ...
                if slice_num < self.config.twap_slice_count - 1 and executed_quantity < abs_quantity:
    ...
    async def _execute_limit_order_with_guard(self, symbol: str, side: OrderSide, 
                                           quantity: Decimal) -> Tuple[Decimal, Decimal, Decimal]:
        """Execute limit order with spread guard"""
        ...
        if spread_bps > self.config.max_spread_bps:
            logger.warning(f"Spread too wide ({spread_bps:.1f}bps > {self.config.max_spread_bps}bps), using market order")
        ...
        if filled_qty > 0:
            avg_price = Decimal(str(order_result.get('fills', [{}])[0].get('price', '0')))
            proceeds = filled_qty * avg_price
            fees = sum(Decimal(str(fill.get('commission', '0'))) for fill in order_result.get('fills', []))
    ...
    async def _execute_market_order(self, symbol: str, side: OrderSide, 
        ...
        try:
            ...
            for symbol, position in state.positions.items():
                if not position.is_flat:
                    ...
                    result = await self.flattener.flatten_position(symbol, position.quantity)
                    ...
                    if result.success:
                        ...
                        logger.error(f"Failed to close position {symbol}: {result.error_message}")
...
async def execute_flatten_operation(client: BinanceClient, state: BotState, 
                                  reason: str, config: FlattenConfig = None) -> FlattenResult:
