...
class MarketConditions:
    ...
    def is_favorable_for_grid(self) -> bool:
        ...
        return (
            self.regime in [MarketRegime.CALM, MarketRegime.NORMAL] and
            self.trend_direction == "sideways" and
            not self.correlation_breakdown
        )
...
class AdvancedSafetyManager:
    ...
    def start_monitoring(self, session_id: str, symbol: str):
        ...
        self._create_alert(AlertType.INFO, "Safety", "Monitoring Started", 
    ...
    def stop_monitoring(self, session_id: str):
        ...
        self._create_alert(AlertType.INFO, "Safety", "Monitoring Stopped",
                          f"Advanced safety monitoring deactivated for {session_id}")
    
    def update_market_data(self, symbol: str, price: Decimal, volume: Decimal, timestamp: datetime):
    ...
    def check_session_safety(self, session_id: str, current_pnl: Decimal, 
                           position_size: Decimal, trades_count: int) -> Tuple[bool, List[SafetyAlert]]:
        """Check session safety and return alerts"""
        alerts = []
        is_safe = True
        
        # Calculate risk metrics
        risk_metrics = self._calculate_risk_metrics(session_id, current_pnl, position_size)
        ...
        if risk_metrics.current_drawdown > self.risk_limits['max_drawdown']:
            ...
            alert = self._create_alert(
                AlertType.CRITICAL, "Risk", "Drawdown Limit Exceeded",
                f"Session {session_id} exceeded max drawdown: {risk_metrics.current_drawdown:.1%}",
                session_id=session_id, risk_level=RiskLevel.CRITICAL, requires_action=True
            )
            alerts.append(alert)
        ...
        if position_size > self.risk_limits['max_position_size']:
            is_safe = False
            alert = self._create_alert(
                AlertType.WARNING, "Risk", "Position Size Warning",
                f"Position size ${position_size} approaching limit",
                session_id=session_id, risk_level=RiskLevel.HIGH
            )
            alerts.append(alert)
        ...
        if risk_level in [RiskLevel.EXTREME, RiskLevel.CRITICAL]:
            alert = self._create_alert(
                AlertType.WARNING, "Market", "High Risk Conditions",
    ...
    def _calculate_risk_metrics(self, session_id: str, current_pnl: Decimal, 
                               position_size: Decimal) -> RiskMetrics:
        """Calculate current risk metrics"""
        # Get existing metrics or create new
        existing = self.session_metrics.get(session_id)
        if existing:
            max_drawdown = max(existing.max_drawdown, abs(min(Decimal('0'), current_pnl)))
        else:
            max_drawdown = abs(min(Decimal('0'), current_pnl))
        
        # Calculate current drawdown
        current_drawdown = abs(min(Decimal('0'), current_pnl)) / max(position_size, Decimal('1'))
        ...
        return RiskMetrics(
            ...
            sharpe_ratio=max(-2.0, min(3.0, float(current_pnl) / max(float(position_size), 1) * 10)),
    ...
    def _update_market_conditions(self, symbol: str):
        ...
        prices = [float(p[1]) for p in self.price_history[symbol][-50:]]  # Last 50 prices
    ...
    def _create_alert(self, alert_type: AlertType, category: str, title: str, 
                     message: str, session_id: Optional[str] = None, 
                     risk_level: Optional[RiskLevel] = None, 
                     requires_action: bool = False) -> SafetyAlert:
