...
class GridEngine:
    ...
    def get_active_orders(self) -> List[GridLevel]:
        ...
        return [level for level in self.grid_levels.values() 
                if not level.filled and level.order_id is None]
    
    def on_fill(self, level_index: int, order_id: str, filled_price: Decimal) -> List[GridLevel]:
        """
    ...
    def rebalance_grid(self, current_price: Decimal) -> Tuple[List[GridLevel], List[GridLevel]]:
        """
        ...
        if price_deviation > max_deviation:
            ...
            orders_to_cancel = [level for level in old_levels if level.order_id and not level.filled]
    ...
    def get_grid_status(self) -> Dict:
        ...
        active_orders = sum(1 for level in self.grid_levels.values() if level.order_id and not level.filled)
