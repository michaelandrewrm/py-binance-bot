...
class TradingExecutor:
    ...
    def __init__(self, 
                 ...
                 state_save_path: str = "bot_state.json") -> None:
    ...
    async def start_trading_loop(self, strategy_callback: Optional[Callable] = None) -> None:
        ...
        try:
            while self.running:
                ...
                if safety_action == SafetyAction.EMERGENCY_STOP:
                    ...
                    logger.warning("PAUSE action triggered - skipping strategy execution")
                    self.state.status = BotStatus.PAUSED
                ...
                if strategy_callback:
                    try:
                        ...
                        self.safety_manager.update_metrics(api_errors=self.state.error_count)
    ...
    async def _save_state_if_needed(self) -> None:
        ...
        if (len(self.state.active_orders) > 0 or 
            (datetime.now(timezone.utc) - self.state.last_update).total_seconds() > 300):
    ...
    def _log_safety_status(self, current_action: SafetyAction) -> None:
        ...
        logger.info(f"Metrics - PnL: {metrics['total_pnl']:.2f}, "
                   f"Drawdown: {metrics['max_drawdown']:.2%}, "
                   f"Errors: {metrics['api_errors']}")
    
    @classmethod
    async def from_unified_config(cls, state: BotState, config=None) -> 'TradingExecutor':
        ...
        risk_limits = RiskLimits(
            ...
            max_daily_loss=config.trading.max_position_size * Decimal('0.1'),  # 10% of max position
