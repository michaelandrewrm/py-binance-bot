...
class SizingCalculator:
    ...
    def calculate_max_quantity(
        self, price: Decimal, available_balance: Decimal
    ) -> Decimal:
    ...
    def validate_order(self, price: Decimal, quantity: Decimal) -> Tuple[bool, str]:
        ...
        if quantity < self.exchange_info.min_qty:
            return (
                False,
                f"Quantity {quantity} below minimum {self.exchange_info.min_qty}",
            )
        ...
        if quantity > self.exchange_info.max_qty:
            return (
                False,
                f"Quantity {quantity} above maximum {self.exchange_info.max_qty}",
            )
        ...
        if remainder != 0:
            return (
                False,
                f"Quantity {quantity} not aligned to step size {self.exchange_info.step_size}",
            )
        ...
        if remainder != 0:
            return (
                False,
                f"Price {price} not aligned to tick size {self.exchange_info.tick_size}",
            )
        ...
        if notional < self.exchange_info.min_notional:
            return (
                False,
                f"Notional {notional} below minimum {self.exchange_info.min_notional}",
            )

        return True, ""

    def calculate_fees(
        self, price: Decimal, quantity: Decimal, is_maker: bool = True
    ) -> Dict[str, Decimal]:
        """
        ...
        notional = price * quantity
        base_fee_rate = (
            self.fee_config.maker_fee if is_maker else self.fee_config.taker_fee
        )
    ...
    def calculate_position_value(
        self, quantity: Decimal, current_price: Decimal
    ) -> Dict[str, Decimal]:
