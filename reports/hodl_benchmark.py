...
class HODLBenchmark:
    ...
    def calculate_hodl_performance(self, klines: List[KlineData], 
        ...
        # Maximum drawdown
        max_drawdown, max_drawdown_duration = self._calculate_hodl_drawdown(filtered_klines, position_size)
        
        # Sharpe ratio (assuming 0% risk-free rate)
        sharpe_ratio = float(annualized_return) / annualized_vol if annualized_vol > 0 else 0
        
        # Calmar ratio
        calmar_ratio = float(annualized_return) / abs(float(max_drawdown)) if max_drawdown != 0 else 0
        ...
        return {
            ...
            'fee_config': {
                ...
                'bnb_discount': float(self.fee_config.bnb_discount) if self.fee_config.use_bnb else None
            }
        }
    
    def compare_with_strategy(self, strategy_metrics: Dict, 
        ...
        excess_return = strategy_metrics.get('total_return', 0) - hodl_metrics['total_return']
        excess_annual_return = strategy_metrics.get('annualized_return', 0) - hodl_metrics['annualized_return']
        ...
        drawdown_improvement = abs(hodl_dd) - abs(strategy_dd)  # Positive means strategy had lower drawdown
        ...
        return {
            ...
            'comparison': {
                ...
                'return_ratio': strategy_metrics.get('total_return', 0) / hodl_metrics['total_return'] if hodl_metrics['total_return'] != 0 else float('inf')
            }
        }
    ...
    def generate_benchmark_report(self, comparison_data: Dict) -> str:
        ...
        report.append("-" * 60)
        report.append(f"Total Return           {strategy.get('total_return', 0):+8.2%}   {hodl['total_return']:+8.2%}   {comp['excess_return']:+8.2%}")
        report.append(f"Annualized Return      {strategy.get('annualized_return', 0):+8.2%}   {hodl['annualized_return']:+8.2%}   {comp['excess_annual_return']:+8.2%}")
        report.append(f"Sharpe Ratio           {strategy.get('sharpe_ratio', 0):8.3f}   {hodl['sharpe_ratio']:8.3f}   {comp['sharpe_difference']:+8.3f}")
        report.append(f"Max Drawdown           {strategy.get('max_drawdown', 0):8.2%}   {hodl['max_drawdown']:8.2%}   {comp['drawdown_improvement']:+8.2%}")
        report.append(f"Volatility             {strategy.get('volatility', 0):8.2%}   {hodl['volatility']:8.2%}   N/A")
        report.append("")
        
        # Capital comparison
        report.append("CAPITAL COMPARISON")
        report.append("-" * 30)
        report.append(f"Initial Capital:        ${strategy.get('initial_capital', 0):,.2f}")
        report.append(f"Strategy Final:         ${strategy.get('final_capital', 0):,.2f}")
        report.append(f"HODL Final:             ${hodl['final_capital']:,.2f}")
        report.append(f"Difference:             ${strategy.get('final_capital', 0) - hodl['final_capital']:+,.2f}")
        report.append("")
        
        # Fee analysis for HODL
        report.append("HODL FEE BREAKDOWN")
        report.append("-" * 30)
        report.append(f"Entry Fee:              ${hodl.get('entry_fee_amount', 0):,.2f}")
        ...
        report.append(f"Trading Frequency:      {strategy.get('total_trades', 0) / max(hodl['total_days'], 1):.2f} trades/day")
        report.append("")
        ...
        if comp['strategy_outperformed']:
            ...
            report.append(f"The strategy achieved {comp['excess_return']:+.2%} excess return")
            if comp['sharpe_difference'] > 0:
                report.append(f"with {comp['sharpe_difference']:+.3f} better risk-adjusted performance")
        else:
            report.append("❌ STRATEGY UNDERPERFORMED HODL")
            report.append(f"The strategy underperformed by {comp['excess_return']:+.2%}")
        ...
        if comp['drawdown_improvement'] > 0:
            report.append(f"✅ Strategy had {comp['drawdown_improvement']:.2%} lower maximum drawdown")
        else:
            report.append(f"⚠️  Strategy had {abs(comp['drawdown_improvement']):.2%} higher maximum drawdown")
    ...
    def _filter_klines_by_date(self, klines: List[KlineData],
                             start_date: Optional[datetime],
                             end_date: Optional[datetime]) -> List[KlineData]:
    ...
    def _calculate_hodl_drawdown(self, klines: List[KlineData], 
                               position_size: Decimal) -> Tuple[Decimal, int]:
    ...
    def _empty_hodl_metrics(self) -> Dict:
        ...
        return {
            ...
            'max_drawdown_duration': 0, 'sharpe_ratio': 0,
            'calmar_ratio': 0, 'initial_capital': 0,
            'final_capital': 0, 'total_days': 0
        }
...
class BenchmarkSuite:
    ...
    def run_all_benchmarks(self, klines: List[KlineData],
                          initial_capital: Decimal,
                          strategy_metrics: Dict) -> Dict:
        ...
        results['sma_benchmark'] = self._calculate_sma_benchmark(klines, initial_capital)
        
        # Random Trading Benchmark (placeholder)
        results['random_benchmark'] = self._calculate_random_benchmark(klines, initial_capital)
        
        return results
    
    def _calculate_sma_benchmark(self, klines: List[KlineData], 
                               initial_capital: Decimal) -> Dict:
    ...
    def _calculate_random_benchmark(self, klines: List[KlineData],
                                  initial_capital: Decimal) -> Dict:
    ...
    def generate_benchmark_summary(self, benchmark_results: Dict) -> str:
        ...
        report.append("-" * 40)
        report.append(f"{'Benchmark':<20} {'Strategy Return':<15} {'Benchmark Return':<15} {'Outperformed':<12}")
        report.append("-" * 65)
        
        for benchmark_name, benchmark_data in benchmark_results.items():
            if benchmark_name == 'hodl':
                strategy_return = benchmark_data['strategy_metrics'].get('total_return', 0)
                benchmark_return = benchmark_data['hodl_metrics']['total_return']
                outperformed = "✅ Yes" if strategy_return > benchmark_return else "❌ No"
            else:
            ...
            report.append(f"{benchmark_name.upper():<20} {strategy_return:<15.2%} {benchmark_return:<15.2%} {outperformed:<12}")
...
def quick_hodl_comparison(strategy_metrics: Dict, klines: List[KlineData],
                        initial_capital: Decimal, fee_config: Optional[FeeConfig] = None) -> str:
    """Quick HODL comparison and report generation"""
    benchmark = HODLBenchmark(fee_config)
    comparison = benchmark.compare_with_strategy(strategy_metrics, klines, initial_capital)
    return benchmark.generate_benchmark_report(comparison)

def calculate_hodl_baseline(symbol: str, start_date: datetime, end_date: datetime,
...
if __name__ == "__main__":
    ...
    logger.info("Use this module to compare trading strategies against buy-and-hold performance")
