...
class ReportGenerator:
    ...
    def generate_backtest_report(self, result: BacktestResult, 
                               comparison_results: Optional[List[BacktestResult]] = None) -> str:
        ...
        html_content = self._create_backtest_html(
            result, equity_chart, trades_chart, monthly_returns_chart, comparison_results
        )
    ...
    def generate_optimization_report(self, optimization_results: Dict, 
                                   strategy_name: str, symbol: str) -> str:
        ...
        parameter_importance_chart = self._create_parameter_importance_chart(optimization_results)
        
        # Create HTML content
        html_content = self._create_optimization_html(
            optimization_results, strategy_name, symbol,
            optimization_chart, parameter_importance_chart
        )
    ...
    def generate_trading_session_report(self, trades: List[TradeRecord],
        ...
        session_metrics = self._calculate_session_metrics(trades, session_start, session_end)
        ...
        html_content = self._create_session_html(
            trades, session_metrics, session_start, session_end,
            pnl_chart, volume_chart
        )
    ...
    def _calculate_session_metrics(self, trades: List[TradeRecord],
                                 session_start: datetime, session_end: datetime) -> Dict:
        """Calculate metrics for a trading session"""
        
        # Filter trades to session period
        session_trades = [t for t in trades 
                         if session_start <= t.timestamp <= session_end]
        ...
        total_volume = sum(float(trade.quantity) * float(trade.price) for trade in session_trades)
        ...
        avg_loss = sum(float(t.pnl or 0) for t in losing_trades) / len(losing_trades) if losing_trades else 0
        profit_factor = abs(avg_win / avg_loss) if avg_loss != 0 else float('inf')
        
        # Trading frequency
        session_duration = (session_end - session_start).total_seconds() / 3600  # hours
        trades_per_hour = len(session_trades) / session_duration if session_duration > 0 else 0
    ...
    def _empty_session_metrics(self) -> Dict:
        ...
        return {
            'total_trades': 0, 'total_pnl': 0, 'total_volume': 0,
            'total_fees': 0, 'net_pnl': 0, 'winning_trades': 0,
            'losing_trades': 0, 'win_rate': 0, 'avg_win': 0,
            'avg_loss': 0, 'profit_factor': 0, 'trades_per_hour': 0,
    ...
    def _create_trades_chart(self, trades: List[TradeRecord]) -> str:
        ...
        try:
            ...
            # Plot trades
            if winning_times:
                ax.scatter(winning_times, winning_pnls, color='green', alpha=0.6, s=50, label='Winning Trades')
            if losing_times:
                ax.scatter(losing_times, losing_pnls, color='red', alpha=0.6, s=50, label='Losing Trades')
    ...
    def _create_monthly_returns_chart(self, equity_curve) -> str:
        ...
        try:
            ...
            monthly_returns = df.groupby(pd.Grouper(freq='M'))['value'].last().pct_change().dropna()
            ...
            bars = ax.bar(range(len(monthly_returns)), monthly_returns.values * 100, color=colors, alpha=0.7)
    ...
    def _create_optimization_chart(self, optimization_results: Dict) -> str:
        ...
        try:
            ...
            ax.plot(trials, values, 'o-', alpha=0.6, label='Trial Values', color='lightblue')
            ax.plot(trials, best_values, '-', linewidth=2, label='Best Value So Far', color='darkblue')
            
            # Formatting
            ax.set_title('Hyperparameter Optimization Progress', fontsize=16, fontweight='bold')
    ...
    def _create_backtest_html(self, result: BacktestResult, equity_chart: str,
                            trades_chart: str, monthly_chart: str,
                            comparison_results: Optional[List[BacktestResult]] = None) -> str:
...
<body>
    ...
    def _create_optimization_html(self, optimization_results: Dict, strategy_name: str,
                                symbol: str, optimization_chart: str, importance_chart: str) -> str:
...
<body>
    ...
    def _create_session_html(self, trades: List[TradeRecord], metrics: Dict,
                           session_start: datetime, session_end: datetime,
                           pnl_chart: str, volume_chart: str) -> str:
