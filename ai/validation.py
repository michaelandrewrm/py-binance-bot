...
class WalkForwardValidator:
    ...
    def __init__(self, train_window_days: int = 252, test_window_days: int = 63,
                 step_days: int = 21, min_trades: int = 10):
    ...
    def generate_periods(self, start_date: datetime, end_date: datetime) -> List[ValidationPeriod]:
    ...
    async def validate_strategy(self, strategy_class, market_data: List[KlineData],
                               optimization_function=None) -> List[ValidationResult]:
        """
        ...
        for period in periods:
            logger.info(f"Validating period {period.period_id}: {period.test_start} to {period.test_end}")
            
            # Split data
            train_data = [k for k in market_data 
                         if period.train_start <= k.open_time <= period.train_end]
            test_data = [k for k in market_data 
    ...
    def _calculate_metrics(self, signals: List[Dict], klines: List[KlineData]) -> PerformanceMetrics:
        """Calculate performance metrics from signals"""
        # Simplified metrics calculation
        trade_pnls = [signal.get('pnl', Decimal('0')) for signal in signals 
        ...
        return PerformanceMetrics(
            ...
            sharpe_ratio=total_return / Decimal('0.2') if total_return != 0 else Decimal('0'),
            calmar_ratio=total_return / Decimal('0.1') if total_return != 0 else Decimal('0'),
    ...
    def _calculate_overfitting_score(self, train_metrics: PerformanceMetrics, 
                                   test_metrics: PerformanceMetrics) -> float:
        """Calculate overfitting score (lower is better)"""
        if train_metrics.total_return == 0:
            return 1.0
        
        # Performance degradation from train to test
        performance_ratio = float(test_metrics.total_return / train_metrics.total_return)
    ...
    def _empty_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            total_return=Decimal('0'), annualized_return=Decimal('0'),
            cumulative_return=Decimal('0'), volatility=Decimal('0'),
            max_drawdown=Decimal('0'), max_drawdown_duration=0,
            sharpe_ratio=Decimal('0'), calmar_ratio=Decimal('0'),
            sortino_ratio=Decimal('0'), total_trades=0, winning_trades=0,
            losing_trades=0, win_rate=Decimal('0'), profit_factor=Decimal('0'),
            average_win=Decimal('0'), average_loss=Decimal('0'),
            largest_win=Decimal('0'), largest_loss=Decimal('0'),
            utilization=Decimal('0'), turnover=Decimal('0')
        )
...
class RollingWindowValidator:
    ...
    def validate_parameters(self, strategy_class, market_data: List[KlineData],
            ...
            for param_value in param_values:
                ...
                if window_performances:
                    parameter_results[param_name][param_value] = {
                        ...
                        'stability_score': self._calculate_stability_score(window_performances)
                    }
    ...
    def _generate_windows(self, market_data: List[KlineData]) -> List[List[KlineData]]:
        ...
        while start_idx < len(market_data):
            ...
            window_data = [k for k in market_data[start_idx:] 
                          if k.open_time <= window_end]
            ...
            start_idx = next((i for i, k in enumerate(market_data) 
                            if k.open_time >= next_start_time), len(market_data))
...
class ValidationReporter:
    ...
    def _generate_recommendations(self, results: List[ValidationResult]) -> List[str]:
        ...
        if consistency_rate < 0.6:
            recommendations.append("Strategy shows low consistency across periods. Consider parameter optimization.")
        
        # Check overfitting
        avg_overfitting = np.mean(overfitting_scores)
        if avg_overfitting > 0.3:
            recommendations.append("High overfitting detected. Consider regularization or simpler models.")
        ...
        if performance_ratio < 0.5:
            recommendations.append("Significant performance degradation from train to test. Review strategy logic.")
...
async def quick_walkforward_validation(strategy_class, market_data: List[KlineData]) -> Dict:
...
def parameter_stability_analysis(strategy_class, market_data: List[KlineData],
                               parameter_ranges: Dict) -> Dict:
