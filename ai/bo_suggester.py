...
class BayesianOptimizer:
    ...
    def __init__(self, hyperparameters: List[HyperParameter], 
                 objective_function: Callable, n_initial_points: int = 10):
    ...
    async def optimize(self, n_trials: int = 50, 
        ...
        for trial_id in range(n_trials):
            ...
            try:
                ...
                if self.best_result is None or objective_value > self.best_result.objective_value:
                    ...
                    logger.info(f"New best result: {objective_value:.4f} at trial {trial_id}")
        ...
        logger.info(f"Optimization completed. Best score: {self.best_result.objective_value:.4f}")
    ...
    def _random_sample(self) -> Dict[str, Any]:
        ...
        for hp in self.hyperparameters:
            if hp.param_type == 'float':
                if hp.log_scale:
                    value = np.exp(np.random.uniform(np.log(hp.bounds[0]), np.log(hp.bounds[1])))
                else:
...
class StrategyOptimizer:
    ...
    def __init__(self, strategy_class, market_data, initial_capital: Decimal = Decimal('10000')):
        self.strategy_class = strategy_class
        self.market_data = market_data
        self.initial_capital = initial_capital
    
    async def optimize_strategy(self, hyperparameters: List[HyperParameter],
        ...
        async def objective_function(parameters: Dict) -> Tuple[float, Dict]:
            ...
            try:
                ...
                total_pnl = sum(signal.get('pnl', Decimal('0')) for signal in signals 
                               if signal.get('pnl') is not None)
                
                # Calculate metrics
                total_trades = sum(1 for signal in signals if signal.get('pnl') is not None)
                win_rate = sum(1 for signal in signals if signal.get('pnl', 0) > 0) / max(total_trades, 1)
...
async def run_strategy_optimization_example():
    ...
    hyperparams = [
        ...
        HyperParameter('param3', 'categorical', choices=['option1', 'option2', 'option3'])
    ]
