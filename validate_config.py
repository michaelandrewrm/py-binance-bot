...
def check_env_file() -> Dict[str, Any]:
    ...
    if not env_file.exists():
        result["warnings"].append(".env file not found - using environment variables and defaults")
    ...
    try:
        ...
        for section in required_sections:
            if section not in config:
                result["errors"].append(f"Missing required section '{section}' in config.yaml")
                result["status"] = "error"
            else:
...
def check_dependencies() -> Dict[str, Any]:
    ...
    if missing_packages:
        ...
        pip_packages = [pkg.replace("_", "-") if pkg == "pydantic_settings" else pkg for pkg in missing_packages]
        pip_packages = ["PyYAML" if pkg == "yaml" else pkg for pkg in pip_packages]
        pip_packages = ["python-dotenv" if pkg == "dotenv" else pkg for pkg in pip_packages]
        result["messages"].append("Install missing packages with: pip install " + " ".join(pip_packages))
...
def check_configuration_loading() -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(settings, 'validate_configuration'):
            ...
            if issues:
                result["warnings"].extend([f"Configuration issue: {issue}" for issue in issues])
            else:
...
def main():
    ...
    if all(r["status"] == "success" for r in all_results):
        ...
        print("⚠️  Warnings found. Configuration will work but consider addressing the warnings.")
        return 0
