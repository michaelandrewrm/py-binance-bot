...
class SQLiteRepository:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_orders_run_ts ON orders(run_id, ts_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_orders_symbol_side ON orders(symbol, side, ts_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(status, ts_utc)"
            )
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_fills_symbol_side ON fills(symbol, side, ts_utc)"
            )
            cursor.execute(
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_equity_run_ts ON equity_curve(run_id, ts_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_equity_ts ON equity_curve(ts_utc)"
            )

            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_klines_symbol_timeframe ON klines(symbol, timeframe, open_time_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_klines_time_range ON klines(open_time_utc, close_time_utc)"
            )

            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_trades_symbol_time ON trades(symbol, timestamp_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_trades_strategy ON trades(strategy_name, timestamp_utc)"
            )

            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_backtest_strategy ON backtest_results(strategy_name, created_at_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_backtest_symbol ON backtest_results(symbol, start_date_utc)"
            )

            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_bot_states_time ON bot_states(timestamp_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_bot_states_manual_resume ON bot_states(requires_manual_resume, timestamp_utc)"
            )

            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_metrics_run_time ON performance_metrics(run_id, timestamp_utc)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_metrics_strategy_metric ON performance_metrics(strategy_name, metric_name, timestamp_utc)"
            )

            conn.commit()
            logger.info(
                "Database initialized with hardened persistence, foreign keys, and performance indices"
            )
        ...
        with self.get_connection() as conn:
            ...
            """,
                (
                    ...
                    result.symbol,
                    utc_isoformat(result.start_date),
                    utc_isoformat(result.end_date),
                    float(result.initial_capital),
                    float(result.final_capital),
                    float(result.total_return),
                    float(result.metrics.sharpe_ratio),
        ...
        with self.get_connection() as conn:
            ...
            """,
                (
                    ...
                    float(order_data.get("cummulativeQuoteQty", 0))
                    / float(order_data.get("executedQty", 1)),
                    float(order_data.get("fee", 0)),
                    utc_isoformat(
                        datetime.fromtimestamp(
                            order_data["time"] / 1000, tz=timezone.utc
                        )
                    ),
                    utc_isoformat(
                        datetime.fromtimestamp(
                            order_data.get("updateTime", order_data["time"]) / 1000,
                            tz=timezone.utc,
                        )
                    ),
    ...
    def load_fills(
        self, run_id: str, symbol: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def update_trading_run_status(
        self, run_id: str, status: str, final_capital: Optional[float] = None
    ):
    ...
    def cleanup_old_data(self, days_to_keep: int = 90):
        ...
        with self.get_connection() as conn:
            ...
            # Clean old klines
            cursor.execute(
                "DELETE FROM klines WHERE created_at_utc < ?", (cutoff_date,)
            )
            ...
            cursor.execute(
                "DELETE FROM trading_runs WHERE end_time_utc < ? AND status = 'completed'",
                (cutoff_date,),
            )
        ...
        logger.info(
            f"Cleaned up {klines_deleted} klines, {states_deleted} bot states, {runs_deleted} trading runs"
        )
...
class HybridRepository:
    ...
    def load_klines(
        self, symbol: str, interval: str, prefer_parquet: bool = True, **kwargs
    ) -> List[KlineData]:
